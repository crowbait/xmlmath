...
**XMLMath** is a powerful command-line tool for performing arithmetic operations directly on XML files. It allows batch modifications of attributes and values using a flexible set of filters and modifiers — making it ideal for automated XML data transformations.
...
With the exception of xmllint, these are already available on most distributions.
...
chmod +x xmlmath
```
You can also run it directly; this ensures you're always working with the latest release and doesn't put the file on your drive.
```bash
...
#### Examples
This multiplies the value (between tags) of all XML elements named "val" in `input.xml` by 2, writing the results back to the same file:
```bash
./xmlmath value multiply 2 -file input.xml --inplace val
```
...
```
You can run arbitrary operations on the found values, providing they can be expressed in *awk*-compatible syntax - in this case, all attribute values (see regex) will be replaces by their root, added to their squared value.
**The token `x` will be substituted for the found value**:
```bash
./xmlmath attr expression 'sqrt(x)+x*x' --file in.xml -r '.*'
```
You can get more examples from the test: `test.sh` list commands.
...
- `expression` or `e` treats the modifier as an expression. It must be *awk*-compatible syntax and should be quoted.
